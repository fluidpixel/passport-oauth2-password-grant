...
function PasswordGrantStrategy(options, verify) {
  ...
  if (!verify) {
    throw new TypeError('PasswordGrantStrategy requires a verify callback');
  }
  if (!options.tokenURL) {
    throw new TypeError('PasswordGrantStrategy requires a tokenURL option');
  }
  if (!options.clientID) {
    throw new TypeError('PasswordGrantStrategy requires a clientID option');
  }
...
PasswordGrantStrategy.prototype.authenticate = function (req, options) {
  ...
  var self = this;
  options = self._authDetails || options || {};
  console.log('options:', options);

  if (!options.username) {
    throw new TypeError('PasswordGrantStrategy requires a username param');
  }
  if (!options.password) {
    throw new TypeError('PasswordGrantStrategy requires a password param');
  }

  // console.log('_authDetails:', self._authDetails);
  var params = {};
